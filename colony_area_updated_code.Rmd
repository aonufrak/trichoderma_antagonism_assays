...
juglone_summaries<-ggplot(data=beauveria.trichoderma.juglone_inhibition_frames,aes(x=Concentration,y=percent_inhibition))+
geom_boxplot(outlier.shape=NA)+
  geom_jitter()+
  facet_wrap(.~isolate,nrow = 4,ncol=4)+
  geom_hline(yintercept = c(0,50),linetype=2)+
 geom_text(data=tricho_juglone_sig_codes,aes(x=Concentration,y=110,label=Code,fontface="bold"),inherit.aes = FALSE)+
  scale_y_continuous(limits=c(-50,110),breaks=c(-50,-25,0,25,50,75,100))+
  ylab("Percent Inhibition (%)")+
  xlab(bquote(bold('Concentration'~(mg~'*'~ml^-1))))+
  theme(panel.grid=element_blank(),panel.background = element_blank(),panel.border = element_rect(fill=NA),strip.background = element_blank(),strip.text.x = element_text(
        size = 12, face = "bold"
        ),axis.text=element_text(face="bold",size=12),axis.title = element_text(size=12,face="bold"))

juglone_summaries

#ggsave("/Users/aarononfurak/Library/CloudStorage/GoogleDrive-onufrak.aaron@gmail.com/My Drive/utk_project/trichoderma_antagonism_manuscript/manuscript_versions/manuscript_biocontrol_journal_revision_03//figures/figure_03.eps",juglone_summaries,dpi = 1200,height=12,width=12)
...
dual_plate_round_1<-ggplot(data=dual_plate_trial_1,aes(x=Rating))+
 geom_histogram(color="black",binwidth = 0.5)+
  ...
  facet_wrap(.~Isolate,scales="free_x",nrow =3 ,ncol=5)+
    theme(panel.grid=element_blank(),panel.background = element_blank(),panel.border = element_rect(fill=NA),strip.background = element_blank(),axis.title=element_text(size=12,face="bold"),axis.text=element_text(size=12,face="bold"),strip.text.x = element_text(
        size = 12, face = "bold"
        ))+
...
ggsave("/Users/aarononfurak/Library/CloudStorage/GoogleDrive-onufrak.aaron@gmail.com/My Drive/utk_project/trichoderma_antagonism_manuscript/manuscript_versions/manuscript_biocontrol_journal_revision_03/figures/figure_04.eps",dual_plate_assay_plot,dpi = 1200,height=10,width=14)
...
poison_agar_area<-read.table("/Users/aarononfurak/Library/CloudStorage/GoogleDrive-onufrak.aaron@gmail.com/My Drive/utk_project/trichoderma_antagonism_manuscript/manuscript_versions/manuscript_biocontrol_journal_revision_03/code_for_manuscript_revision/raw_data_with_excess_control.txt",header=TRUE,sep='\t',na.strings = "NA")
...
nrow(poison_percent_inhibition_unautoclave)
...
# Plotting results in ggplot
autoclave_pi<-ggplot(data=poison_percent_inhibition_autoclave,aes(x=iso_code,y=percent_inhibition))+
geom_boxplot(outlier.shape=NA)+
...
# Plotting results in ggplot
unautoclave_pi<-ggplot(data=poison_percent_inhibition_unautoclave,aes(x=iso_code,y=percent_inhibition))+
geom_boxplot(outlier.shape=NA)+
  geom_point(position = position_jitterdodge(jitter.width = 0.05),color="black",aes(shape=Isolate))+
  facet_wrap(.~Percentage,nrow = 1,ncol=3)+
  geom_hline(yintercept = c(0,50),linetype=2)+
  theme(panel.grid=element_blank(),panel.background = element_blank(),panel.border = element_rect(fill=NA),strip.background = element_blank(),axis.title.x = element_blank(),axis.title=element_text(size=12,face="bold"),axis.text=element_text(size=12,face="bold"),strip.text.x = element_text(
        size = 12, face = "bold"
        ))+
...
ggsave("/Users/aarononfurak/Library/CloudStorage/GoogleDrive-onufrak.aaron@gmail.com/My Drive/utk_project/trichoderma_antagonism_manuscript/manuscript_versions/manuscript_biocontrol_journal_revision_03/figures/figure_05.eps",metabolite_summary_results,dpi = 1200,height=10,width=18,bg="white")
...
poison_percent_inhibition_df_no_pda<-poison_percent_inhibition_df[grep("PDA|water",poison_percent_inhibition_df$tricho,invert=TRUE),]

# Performing ANOVA
autoclave_aov<-aov(percent_inhibition~heat*tricho*Percentage*Isolate,data=poison_percent_inhibition_df_no_pda)
